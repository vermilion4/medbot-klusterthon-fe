  ...
  const handleNextQuestion = (answer) => {
    ...
    if (startRecording && currentQuestionIndex <= questionsList.length) {
      ...
      setUserAnswers((prevAnswers) => [
        ...prevAnswers,
        { question: questionsList?.[currentQuestionIndex-1]?.question, answer:processedAnswer },
      ]);
    }
        ...
        <div>
            ...
            <div className='-mt-[20px] ml-[60px] border border-grey-200 shadow-lg w-[300px] p-4 rounded-lg'>
              {currentQuestionIndex === 0
                ...
                : "Thank You, Tola. I have enough info for now and can help you find out what’s going on. Just start a symptom assesment"}
            </div>
          </div>
          ...
          <div className='-mt-[20px] mr-[60px] border border-grey-200 shadow-lg w-[300px] p-4 rounded-lg flex flex-col gap-2'>
            {currentQuestionIndex === 0 ? (
              ...
              ) : questionsList?.[currentQuestionIndex - 1]?.type === 'text' ? (
                <input
                  type="text"
                  placeholder={questionsList?.[currentQuestionIndex - 1]?.placeholder}
                onKeyPress={handleKeyPress}
                  className={'w-full p-2 border rounded-md'}
                />
              ) : questionsList?.[currentQuestionIndex - 1]?.type === 'date' ? (
                <input
                  type="date"
